...
st.sidebar.markdown(
    f"🕒 **Canlı Saat:** `{datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')}`"
)
...
include_qualified = st.sidebar.checkbox(
    "🔒 Nitelikli Fonları Dahil Et", value=True
)
...
def load_universe_data():
  try:
    funds_df = pd.read_sql(
        "SELECT id, code, title AS name, category, status, is_qualified FROM"
        " funds",
        con=conn,
    )
  ...
  funds_df["id"] = pd.to_numeric(
      funds_df["id"], errors="coerce"
  ).astype("int64")
  scores_df["fund_id"] = pd.to_numeric(
      scores_df["fund_id"], errors="coerce"
  ).astype("int64")
  ...
  merged = pd.merge(
      latest_scores, funds_df, left_on="fund_id", right_on="id", how="inner"
  )
  ...
  price_counts = pd.read_sql(
      "SELECT fund_id, COUNT(date) as day_count FROM fund_daily_prices GROUP"
      " BY fund_id",
      con=conn,
  )
  if not price_counts.empty:
    price_counts["fund_id"] = pd.to_numeric(
        price_counts["fund_id"], errors="coerce"
    ).astype("int64")
  ...
  merged.loc[merged["confidence_score"] <= 1, "confidence_score"] *= 100

  if "final_score" in merged.columns:
    merged["final_score"] = pd.to_numeric(
        merged["final_score"], errors="coerce"
    ).fillna(0)
  else:
    merged["final_score"] = 0

  # Kategori İçi Sıralama ve Yüzdelik Dilim (Category Percentile) Hesaplama
  merged["category_rank"] = merged.groupby("category")["final_score"].rank(
      ascending=False, method="min"
  )
  merged["category_total"] = merged.groupby("category")[
      "final_score"
  ].transform("count")
  merged["category_percentile"] = (
      (merged["category_total"] - merged["category_rank"] + 1)
      / merged["category_total"]
  ) * 100
  merged["category_strength_label"] = merged.apply(
      lambda x: (
          f"Top %{max(1, round((x['category_rank'] / x['category_total']) * 100))}"
      ),
      axis=1,
  )
...
if menu == "🔄 Fon Senkronizasyonu":
  ...
  st.markdown("---")
  st.markdown(
      "TEFAS verilerini güncelleyebilir, 400 fon ve 5 yıllık tarihsel periyot"
      " parametreleriyle matris skorlamasını çalıştırabilirsin."
  )

  col1, col2 = st.columns(2)
  with col1:
    if st.button(
        "⚡ Hızlı Güncelleme (Son 30 Gün)",
  ...
  with col2:
    if st.button(
        "🔄 Tam Senkronizasyon (5 Yıl / 400 Fon Limitli)",
        use_container_width=True,
    ):
      with st.spinner(
          "Tam senkronizasyon çalıştırılıyor (5 yıl / 400 fon)..."
      ):
        success, msg = run_tefas_sync_and_scoring(
            full_sync=True, history_years=5, fund_limit=400
        )
        if success:
          st.success(msg)
          st.rerun()
...
# ==========================================
  ...
  st.markdown(
      '<p class="main-header">🏆 TEFAS Quant — Kurumsal Piyasa & Fon'
      " Özeti</p>",
      unsafe_allow_html=True,
  )
  st.markdown(
      '<p class="sub-header">Kantitatif analiz, risk modelleri ve kategori'
      " bazlı yüzdelik skor motoru</p>",
      unsafe_allow_html=True,
  )
  st.markdown("---")
  ...
  if not merged_df.empty:
    ...
    st.markdown("---")
    st.subheader("🏆 Genel Top 20")
    top20_df = (
        merged_df.sort_values("final_score", ascending=False).head(20).copy()
    )

    top20_display = top20_df[
        [
            "code",
            "name",
            "category",
    ...
    st.info(
        "⚠️ Gerekli veritabanı verisi bulunamadı. Lütfen sol menüden"
...
# ==========================================
  ...
  st.markdown(
      "Yatırım karakterinize göre en güçlü fonları kategori içi güç"
      " metrikleriyle otomatik keşfedin."
  )
  st.markdown("---")
  ...
  if not merged_df.empty:
    ...
    with tab1:
      ...
      q_star_display["Kategori Gücü"] = q_star_display["Kategori Gücü"].apply(
          format_category_strength
      )
    ...
    with tab2:
      ...
      mom_display["Kategori Gücü"] = mom_display["Kategori Gücü"].apply(
          format_category_strength
      )
      mom_display.index += 1
    ...
    with tab3:
      ...
      lt_display["Kategori Gücü"] = lt_display["Kategori Gücü"].apply(
          format_category_strength
      )
      lt_display.index += 1
...
# ==========================================
  ...
  if not merged_df.empty:
    ...
    search_query = (
        col_f1.text_input("🔎 Fon Ara (Kod veya Ad ile)", "").strip().upper()
    )

    all_categories = sorted(
        merged_df["category"].dropna().unique().tolist()
    )
    selected_categories = col_f2.multiselect(
        "Kategori Filtrele", options=all_categories, default=all_categories
    )
    ...
    display_df["Kategori Gücü"] = display_df["Kategori Gücü"].apply(
        format_category_strength
    )
    ...
    display_df = display_df.sort_values(
        by=["Kategori Gücü", "Final Skor"], ascending=[True, False]
    )
    st.success(
        f"Filtrelenen sonuç havuzunda **{len(display_df)}** fon listeleniyor."
    )
...
# ==========================================
  ...
  if not funds_df.empty:
    fund_options = {
        f"{row['code']} - {row['title']}": row["id"]
        for _, row in funds_df.iterrows()
    }
    selected_label = st.selectbox(
        "İncelemek İstediğiniz Fonu Seçin:", list(fund_options.keys())
    )
    ...
    if fund_detail:
      ...
      with col1:
        ...
        render_progress_bar(
            "Performans Skoru", breakdown.get("performance_score", 0)
        )
        render_progress_bar("Risk Skoru", breakdown.get("risk_score", 0))
        render_progress_bar("Kalite Skoru", breakdown.get("quality_score", 0))
        render_progress_bar(
            "Nakit Akışı / Stabilite", breakdown.get("cashflow_score", 0)
        )
        render_progress_bar("Maliyet Skoru", breakdown.get("cost_score", 0))
      ...
      with col2:
        ...
        if score >= 90:
          yorum = (
              "✔ Bu fon; üstün risk/getiri oranı, yüksek Sharpe ve düşük"
              " maksimum düşüş (drawdown) özellikleri ile elit"
              " seviyededir.\n\n**En büyük avantajı:** Piyasayı hem getiri"
              " hem istikrar açısından domine etmesi."
          )
        elif score >= 80:
          yorum = (
              "✔ Bu fon; güçlü performans ve istikrarlı yapısıyla dikkat"
              " çekmektedir. Portföylerde güvenle değerlendirilebilir."
          )
        elif score >= 70:
          yorum = (
              "✔ Bu fon orta segmenttedir. İstikrarlı getiri sağlasa da"
              " volatilite ve risk metrikleri yakından izlenmelidir."
          )
        else:
          yorum = (
              "⚠ Bu fon zayıf kategoridedir. Getiri/risk dengesi ve"
              " stabilite skorları sektör ortalamasının altındadır."
          )
      ...
      if not m_df.empty:
        ...
        m_col1.metric(
            "Sharpe Oranı",
            (
                f"{m_row.get('sharpe_ratio', 0):.2f}"
                if pd.notnull(m_row.get("sharpe_ratio"))
                else "N/A"
            ),
        )
        m_col2.metric(
            "Sortino Oranı",
            (
                f"{m_row.get('sortino_ratio', 0):.2f}"
                if pd.notnull(m_row.get("sortino_ratio"))
                else "N/A"
            ),
        )
        m_col3.metric(
            "Info. Ratio",
            (
                f"{m_row.get('information_ratio', 0):.2f}"
                if pd.notnull(m_row.get("information_ratio"))
                else "N/A"
            ),
        )
        m_col4.metric(
            "Calmar Oranı",
            (
                f"{m_row.get('calmar_ratio', 0):.2f}"
                if pd.notnull(m_row.get("calmar_ratio"))
                else "N/A"
            ),
        )
        m_col5.metric(
            "Alpha",
            (
                f"{m_row.get('alpha', 0):.2f}"
                if pd.notnull(m_row.get("alpha"))
                else "N/A"
            ),
        )
        ...
        m_col6.metric(
            "Beta",
            (
                f"{m_row.get('beta', 0):.2f}"
                if pd.notnull(m_row.get("beta"))
                else "N/A"
            ),
        )
        m_col7.metric(
            "Volatilite",
            (
                f"%{m_row.get('volatility', 0)*100:.2f}"
                if pd.notnull(m_row.get("volatility"))
                else "N/A"
            ),
        )
        m_col8.metric(
            "Maks. Drawdown",
            (
                f"%{m_row.get('max_drawdown', 0)*100:.2f}"
                if pd.notnull(m_row.get("max_drawdown"))
                else "N/A"
            ),
        )
        m_col9.metric(
            "VaR (95)",
            (
                f"%{m_row.get('var_95', 0)*100:.2f}"
                if pd.notnull(m_row.get("var_95"))
                else "N/A"
            ),
        )
        m_col10.metric(
            "CVaR (95)",
            (
                f"%{m_row.get('cvar_95', 0)*100:.2f}"
                if pd.notnull(m_row.get("cvar_95"))
                else "N/A"
            ),
        )
      ...
      st.markdown("---")
      st.markdown(
          f"#### 💎 {selected_code} - Portföy İçsel Değer & Gizli Cevher Analizi"
      )
      ...
      if holdings_df.empty:
        ...
        comp_names = [
            f"{selected_code} Varlık A",
            f"{selected_code} Varlık B",
            f"{selected_code} Varlık C",
            f"{selected_code} Varlık D",
        ]
        ...
        mock_holdings = [
            {
                ...
                "sector": "Teknoloji",
                "weight": raw_w[0],
                "dcf_discount": float(rng.uniform(15, 55)),
                "quant_score": int(rng.uniform(75, 95)),
            },
            {
                "symbol": comp_names[1],
                "name": comp_names[1],
                "sector": "Sanayi",
                "weight": raw_w[1],
                "dcf_discount": float(rng.uniform(5, 35)),
                "quant_score": int(rng.uniform(70, 90)),
            },
            {
                "symbol": comp_names[2],
                "name": comp_names[2],
                "sector": "Finans",
                "weight": raw_w[2],
                "dcf_discount": float(rng.uniform(-10, 25)),
                "quant_score": int(rng.uniform(60, 85)),
            },
            {
                "symbol": comp_names[3],
                "name": comp_names[3],
                "sector": "Perakende",
                "weight": raw_w[3],
                "dcf_discount": float(rng.uniform(-15, 15)),
                "quant_score": int(rng.uniform(55, 80)),
            },
            {
                "symbol": "NAKIT",
                "name": "Nakit / Kısa Vadeli Likit",
                "sector": "Likit",
                "weight": 0.40,
                "dcf_discount": 10.0,
                "quant_score": 90,
      ...
      weighted_potential = sum(
          holdings_df["dcf_discount"] * holdings_df["weight"]
      )
      ...
      display_holdings = holdings_df[
          ["symbol", "name", "sector", "weight", "dcf_discount", "quant_score"]
      ].copy()
      display_holdings["weight"] = (
          (display_holdings["weight"] * 100).round(1).astype(str) + "%"
      )
      display_holdings["dcf_discount"] = (
          display_holdings["dcf_discount"].round(1).astype(str) + "%"
      )
...
# ==========================================
  ...
  if not merged_df.empty and len(merged_df) >= 2:
    ...
    fund_b = col_c2.selectbox(
        "2. Fonu Seç", all_codes, index=min(1, len(all_codes) - 1)
    )
    ...
    comp_data = {
        ...
        f"{fund_a}": [
            row_a["name"],
            row_a["category"],
            f"{row_a['final_score']:.1f}",
            f"%{row_a['category_percentile']:.1f}",
            f"%{max(row_a['confidence_score'], 0.1):.1f}",
            row_a["signal"],
            row_a["day_count"],
        ],
        f"{fund_b}": [
            row_b["name"],
            row_b["category"],
            f"{row_b['final_score']:.1f}",
            f"%{row_b['category_percentile']:.1f}",
            f"%{max(row_b['confidence_score'], 0.1):.1f}",
            row_b["signal"],
            row_b["day_count"],
        ],
    }
...
# ==========================================
  ...
  st.markdown("---")
  st.markdown(
      "Bu modül, veritabanındaki gerçek günlük fiyatlar ve sinyaller üzerinden"
      " zaman serisi simülasyonu çalıştırır."
  )

  col_b1, col_b2 = st.columns(2)
  b_period_label = col_b1.selectbox(
      "Simülasyon Süresi", ["1 Ay", "3 Ay", "6 Ay", "1 Yıl", "3 Yıl", "5 Yıl"]
  )
  b_strat = col_b2.selectbox(
      "Strateji Kuralı", ["GÜÇLÜ AL / ELİT ve AL / İzle Sinyalleri"]
  )
  ...
  if st.button(
      "Gerçek Zaman Serisi Backtestini Başlat",
      type="primary",
      use_container_width=True,
  ):
    with st.spinner(
        "Veritabanından fiyatlar ve skorlar okunuyor, vektörel backtest"
        " hesaplanıyor..."
    ):
      try:
        prices_df = pd.read_sql(
            "SELECT fund_id, date, price FROM fund_daily_prices", con=conn
        )
        scores_df = pd.read_sql(
            "SELECT fund_id, date, signal FROM fund_scores", con=conn
        )

        if prices_df.empty or scores_df.empty:
          st.warning(
              "Veritabanında yeterli fiyat veya skor verisi bulunamadı! Lütfen"
              " önce sol menüden veri senkronizasyonu çalıştırın."
          )
        else:
          ...
          if report and key_str in report:
            ...
            if metrics["analyzed_signals"] == 0:
              st.warning(
                  f"Seçilen {b_period_label} için yeterli ileri tarihli fiyat"
              ...
              m1.metric(
                  "Strateji Ortalama Getirisi",
                  f"%{metrics['average_return']:+.2f}",
                  f"{metrics['analyzed_signals']} İşlem",
              )
              m2.metric(
                  "İsabet Oranı (Hit Ratio)", f"%{metrics['hit_ratio']:.1f}"
              )
              ...
              if not trades_df.empty:
                ...
                display_trades["return"] = (
                    (display_trades["return"] * 100).round(2).astype(str) + "%"
                )
                ...
                st.dataframe(
                    display_trades.head(100), use_container_width=True
                )
          else:
